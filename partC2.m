...
f1 = fit(loc1, peaks1, '(a*(exp(-b*x)))*cos(c*x)+d', 'Exclude', peaks1 < 440, 'StartPoint', [45.63, 0.1938, 0.2595, 455.9]);
plot(f1);
figure; scatter(loc2, peaks2); hold on;
f2 = fit(loc2, peaks2, '(a*(exp(-b*x)))*cos(c*x)+d', 'Exclude', peaks2 < 450, 'StartPoint', [45.63, 0.1938, 0.2595, 455.9]);
plot(f2);
figure; scatter(loc3, peaks3); hold on;
f3 = fit(loc3, peaks3, '(a*(exp(-b*x)))*cos(c*x)+d', 'Exclude', peaks3 < 440, 'StartPoint', [45.63, 0.1938, 0.2595, 455.9]);
plot(f3);
